...
fn main() -> ExitCode {
    ...
    match first.as_deref() {
        ...
        Some(argument) if argument == OsStr::new(PREVIEW_ARGUMENT) => {
            println!(
                "VOLPAROSSA fixed supervisor preview: anonymous namespace bootstrap, exact UID/GID mapping, exact self-reexec PID-1 proof, private mounts, fixed pidfd-to-signalfd supervision, the exact new-netns RTNL baseline, a descriptor-anchored stable canonical IPv4 ip_forward value, empty nftables table/chain/rule/set/object/flowtable dumps bracketed by unchanged generation 1, one pinned BOOTSTRAP_READY, one canonical GO, descriptor-relative private-run roots and slots, two distinct live network namespaces published as run-bound nsfs pins, two fixed down-veth pairs each created atomically, exact parent/A/B down-veth delta proof, four fixed /30 IPv4 addresses proved while every veth end remained down, an exact all-IPv6-addrgen-NONE barrier, exact four-end carrier-up activation including noqueue and the kernel-owned IPv4 and IPv6 route side effects, two exact main-table static /32 endpoint routes installed through bounded raw RTM_NEWROUTE and exactly observed, direct veth deletion B then A as the sole route-removal mechanism, parent/endpoint equality proof against retained enumerated baselines before affine route/address/pair owner retirement, ordinary reverse nsfs unmount B then A, and exact reverse filesystem rollback are implemented; forwarding-setting requests, nftables mutations, packets or probes, dataplane or topology readiness, TOPOLOGY_READY, A14, A15, and acceptance evidence remain blocked."
            );
            ExitCode::SUCCESS
        }
        Some(argument) if argument == OsStr::new(RUN_ARGUMENT) => match run_fixed_lifecycle() {
            ...
            Ok(LifecycleOutcome::BlockedAfterEndpointRouteTeardown) => {
                eprintln!(

//! Affine client-hop dispatch and authenticated connection binding for A1c.
...
pub(super) struct PreselectionTransactionState {
    instance: Arc<()>,
    active: Option<OutboundRequestId>,
}
...
/// Detail-free rejection at the client-hop dispatch and connection-binding boundary.
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
#[non_exhaustive]
pub enum PreselectionDispatchError {
    /// The immutable local roles do not permit client-hop dispatch.
    #[error("preselection client dispatch role is disabled")]
    Role,
    /// The canonical request cannot yield the required target and address family.
    #[error("invalid preselection client dispatch request")]
    Request,
    /// No unique current authenticated connection proves the requested native family.
    #[error("preselection client transport provenance is unavailable")]
    Provenance,
    /// A response event does not belong to this exact dispatch.
    #[error("preselection client response correlation failed")]
    Correlation,
    /// Dispatch or response arrival falls outside the monotonic time window.
    #[error("preselection client dispatch time window failed")]
    Time,
    /// The exact single client-hop dispatch slot is already occupied.
    #[error("preselection client dispatch slot is occupied")]
    Busy,
}
...
impl DiscoveryService {
    ...
    pub fn dispatch_preselection_observation(
        ...
        if self.preselection_transaction.active.is_some() {
        ...
        self.preselection_transaction.active = Some(request_id);
    ...
    fn bind_preselection_observation_response_at(
        ...
        if !Arc::ptr_eq(&self.preselection_transaction.instance, &dispatch.instance)
            || self.preselection_transaction.active != Some(dispatch.request_id)
        {
            return Err(PreselectionDispatchError::Correlation);
        }
        ...
        self.preselection_transaction.active = None;
...
mod tests {
    ...
    use libp2p::{
        ...
        identity,
        swarm::{
            AddressChange, ConnectionClosed, FromSwarm, NetworkBehaviour,
    ...
    async fn typed_client_response_event_retains_one_exact_opaque_proof() {
        ...
        let (bound, returned_response) = service
            .bind_preselection_observation_response(dispatch, event)
    ...
    fn public_tokens_are_affine_exact_and_the_bound_proof_is_fully_opaque() {
        let source = include_str!("preselection_transaction.rs");
        ...
        assert_eq!(
            ...
            "instance:Arc<()>,active:Option<OutboundRequestId>,"
        );
        assert_eq!(
            compact(item_body(
                production,
    ...
    fn dispatch_and_bind_derive_every_authoritative_input_internally() {
        ...
        assert!(dispatch.contains("self.preselection_transaction.active = Some(request_id);"));

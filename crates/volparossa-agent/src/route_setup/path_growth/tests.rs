//! Explicit health-input lifecycle tests, not evidence of a real third MPQUIC datapath.
...
impl Fixture {
    fn new() -> Self {
        Self {
            health: ProductionMpquicPathHealth::new(&[1, 2], [3], wall(0)).unwrap(),
    ...
    fn activate(&mut self, seconds: u64) {
        self.health.record_activation(3, wall(seconds)).unwrap();
        self.growth
            .activated(3, 2, self.start + Duration::from_secs(seconds));
    }
}

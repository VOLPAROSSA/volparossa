...
- [ ] Native MPQUIC API v6 preflights an exact role/process lifetime, targets every later operation to that instance, requires nonce plus canonical-request digest response correlation, and consumes exactly one operation-bound UDP descriptor for `AddPath` or `StartExitSession` and zero otherwise. Start requests bind the signed reservation/finalize IDs, bearer commitment, certificate digest, and both process instances; Rust and C share exact request/descriptor hash vectors and independently reject bearer/commitment mismatch. Focused tests cover stale-instance without hidden retry, response/assignment shape, socket tuple/flag checks, binding and ownership behavior, stream fragmentation with exactly-one ancillary transfer, incomplete/late/extra descriptors, timeout cleanup, and the dormant exit runtime closing its listener before `exit_listener_orchestration_unavailable`. The clean full-graph API-v6 ASan+UBSan gate passes. Peer-control v4 retains separate zeroizing, non-cloneable one-shot client/exit authorizations. Production still lacks an agent caller, separate role service identities/sockets, product-pool assignment and retention, exact-namespace/disposable-topology proof, monotonic expiry/replay, certificate/name/SPKI/key consistency, trusted helper provenance, and the actual exit backend, so route setup and the launcher remain blocked.

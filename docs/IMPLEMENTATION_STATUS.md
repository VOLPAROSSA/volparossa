...
- [ ] Namespace-local MPTCP/QUIC sockets use typed tag-27 `AcquireTransportSocket` and exactly-one CLOEXEC `SCM_RIGHTS` framing; canonical binding, retry, correlation, close-on-reject and consuming credentialed-FD-to-`OwnedFd` adoption tests pass, including audited minimum-3 `F_DUPFD_CLOEXEC`, CLOEXEC readback and original closure. Internal protocol v2 consumes and drops the worker source before an exact credentialed release record, while missing/wrong/late release closes the adopted FD. A consuming parent validator independently verifies the complete socket shape, and fixed `SIOCGSKNS` safely acquires and types its namespace FD, but production still returns `Unavailable` before network work; worker Acquire dispatch, comparison with the retained pinned namespace, datapath adoption and live route proof remain.

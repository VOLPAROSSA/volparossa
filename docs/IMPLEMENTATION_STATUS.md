...
- [ ] Dormant helper tags 35/28 register one exact runtime-global Prepare intent and reconcile only
  ...
  production executor deliberately refuses both proofs, leaving `MayOwnCustody`, `MayOwnPrepare`,
  or `CleanupConfirmed` byte-identical and blocking Ready; independently, the actor may durably
  settle only never-dispatched `Intent` records in production. Admission is bounded to four
  ...
  settlement; that handle exposes no registration, custody-mark, retirement, settlement or lifecycle
  authority. Raw codec, issuance, custody marking and settlement authority remain private; no
...
- [ ] `HelperEngine` now keeps one armed affine owner across asynchronous PLAN/CALL/COMMIT or exact
  ...
  durable journal/systemd custody, crash/restart recovery or disposable live-root proof. Durable
...
- [ ] systemd services use minimum capabilities and restrictive sandboxing; the shipped helper unit
  ...
  blockers, but a custody-capable production restart-settlement executor remains absent. A dormant
...
- [ ] The v3 lease API exposes only opaque handles and public endpoint material and has no private-key
  ...
  WireGuard lease with kernel-proven key/port and selected direct-underlay IP. The public
  `HelperEngine::new` remains `Unavailable`; no production route-manager caller or disposable
...
- [ ] Typed/pure/fake helper boundaries prove exact public handles, cardinality, TTL, idempotency,
  ...
  functional-alpha backend has no production route-manager caller, and no live end-to-end
  worker/kernel tunnel or datapath proof exists.

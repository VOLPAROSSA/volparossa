...
# Fixed GDB-side adapter for the disposable KVM restart proof.
set -eu
export LC_ALL=C
...
umask 077
[ "$#" -eq 4 ] || exit 64
case $1 in armed|cleanup-confirmed|after-crash|recovery-boundary) ;; *) exit 64 ;; esac
case $2 in
...
exec /usr/bin/setpriv \
    ...
    --regid="$observer_gid" \
    --groups="$observer_gid" \
    -- /run/volparossa-helper-production-ipc-hook restart-observe "$@"

...
# Cross-link the reviewed Debian 13 KVM environment to one MayOwn Relay report.
set -eu
export LC_ALL=C
...
expected_commit=$3
expected_image=$4
invalid() { printf 'invalid helper MayOwn Relay VM environment v1: %s\n' "$1" >&2; exit 1; }
for input in "$environment" "$report"; do
    ...
    jq -e -s 'length == 1' "$input" >/dev/null 2>&1 \
        || invalid 'input is not one JSON value'
    jq -S -c . "$input" | cmp -s - "$input" \
        || invalid 'input is not canonical jq JSON'
done
case $expected_commit in *[!0-9a-f]*|'') invalid 'expected commit is invalid' ;; esac
...
"$here/validate-helper-restart-may-own-custody-relay-evidence-v1.sh" "$report" \
    || invalid 'linked MayOwn Relay report is invalid'
report_hash=$(sha256sum "$report" | awk '{print $1}') \
    || invalid 'report hash is unavailable'
jq -e --arg commit "$expected_commit" --arg image "$expected_image" \
  ...
  exact_keys(["expected_commit","guest","image_release_build","image_sha512",
    "proof_network","report_kind","report_sha256","schema_version","status"])
  and .expected_commit == $commit
  and (.guest | exact_keys(["architecture","cargo_version","debian_version",
  ...
  and .image_sha512 == $image
  and (.proof_network | exact_keys(["external_https","mode"])
    and .external_https == "denied" and .mode == "qemu-user-restrict-on")
  and .report_kind ==
    "volparossa-helper-restart-may-own-custody-relay-vm-environment"
  and .report_sha256 == $report_hash
  and .schema_version == 1 and .status == "PASS"
' "$environment" >/dev/null 2>&1 \
    || invalid 'exact environment cross-link is not satisfied'

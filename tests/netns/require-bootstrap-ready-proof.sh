#!/bin/sh
# Require the live nsfs-pair rollback proof on an unprivileged Debian 13 host.
set -eu
...
fail() {
    printf '%s\n' "live nsfs proof gate failed: $1" >&2
    exit 1
}
...
runner_source=$repository_root/target/live-nsfs-proof/x86_64-unknown-linux-gnu/debug/volparossa-netns-runner
...
proof_tmp=$(mktemp -d /tmp/volparossa-live-nsfs-proof.XXXXXX)
cleanup() {
    rm -rf -- "$proof_tmp"
}
trap cleanup EXIT
...
if ! cmp -s "$proof_tmp/expected-stderr" "$proof_tmp/stderr"; then
    fail 'runner did not report the live nsfs rollback outcome'
fi
...
case $proof_scope in
    vm)
        printf '%s\n' 'Debian 13 VM live nsfs proof gate passed'
        ;;
    additional-bare-metal-local)
        printf '%s\n' 'additional bare-metal local live nsfs proof gate passed'
        ;;
    *) fail 'live nsfs proof scope was not classified' ;;
esac

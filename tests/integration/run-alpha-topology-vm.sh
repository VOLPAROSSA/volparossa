...
usage() {
    printf '%s\n' \
        ...
        '         --expected-commit SHA [--scenario alpha|datapath|reciprocity|local-link|mixed-link|mpquic-growth|mptcp-growth|sharing|download-sharing|wifi-mesh|wifi-link|uplink-link|crash-recovery|content|content-message|content-https|content-provider|content-custody|content-replication|content-mailbox|dns-cache]' \
...
while [ "$#" -gt 0 ]; do
    case $1 in
        ...
        --scenario)
            ...
            scenario=$2
            case $scenario in alpha|datapath|reciprocity|local-link|mixed-link|mpquic-growth|mptcp-growth|sharing|download-sharing|wifi-mesh|wifi-link|uplink-link|crash-recovery|content|content-message|content-https|content-provider|content-custody|content-replication|content-mailbox|dns-cache) ;; *) usage >&2; exit 64 ;; esac
            shift
            ;;
        --image)
            [ "$#" -ge 2 ] || { usage >&2; exit 64; }
...
if __name__ == "__main__":
    ...
    if sys.argv[2] not in ("alpha", "datapath", "reciprocity", "local-link", "mixed-link", "mpquic-growth", "mptcp-growth",
                           "sharing", "download-sharing", "wifi-mesh", "wifi-link", "uplink-link", "crash-recovery", "content", "content-message", "content-https", "content-provider", "content-custody", "content-replication", "content-mailbox", "dns-cache"):
        raise SystemExit(64)
...
package_sha256=$4
scenario=$5
case $scenario in alpha|datapath|reciprocity|local-link|mixed-link|mpquic-growth|mptcp-growth|sharing|download-sharing|wifi-mesh|wifi-link|uplink-link|crash-recovery|content|content-message|content-https|content-provider|content-custody|content-replication|content-mailbox|dns-cache) ;; *) exit 64 ;; esac
cd /home/vpci
...
case $scenario in reciprocity|local-link|mixed-link|mpquic-growth|mptcp-growth|sharing|download-sharing|wifi-link|uplink-link|crash-recovery|content|content-message|content-https|content-provider|content-custody|content-replication|content-mailbox|dns-cache) topology_scenario=$scenario ;; esac

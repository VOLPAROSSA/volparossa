...
def check_native_publication(encoded, raw, publisher, name, content_type, expires):
    # Standalone native-content verification; no authority-round fixture dependency.
    fields = POLICY["object_fields"]
    envelope = fields(encoded, 64 * 1024)
    body = fields(envelope[1], 64 * 1024)
    payload = fields(body[8], 64 * 1024, (5,))
    require(set(envelope) == {1, 2} and set(body) == set(range(1, 9))
        ...
        and set(payload) == set(range(1, 7)) and payload[1] == name.encode() and payload[2] == 1
        and payload[3] == content_type.encode() and payload[4] == len(raw)
        and payload[6] == hashlib.sha256(raw).digest(), "native publication changed signed original")
    offset = 0
    for chunk in payload[5]:
        descriptor = fields(chunk, 128)
        require(set(descriptor) == {1, 2} and len(descriptor[1]) == 32 and 0 < descriptor[2] <= 262144
            and descriptor[1] == hashlib.sha256(raw[offset:offset + descriptor[2]]).digest(),
            "native publication chunk does not bind original bytes")
        offset += descriptor[2]
    require(offset == len(raw), "native publication omitted bytes")
    POLICY["verify_signature"](envelope[1], envelope[2], body[2], b"VOLPAROSSA/native-content-manifest/v1\0")
    return body
...
def check_input(value, raw):
    ...
    for label, payload, content_type in (("source", SOURCE, "text/plain"),
        ...
        body = check_native_publication(manifest, payload, publisher, "reasoning-principle-" + label,

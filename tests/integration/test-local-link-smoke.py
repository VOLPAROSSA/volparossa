...
for index, node in enumerate(module.INTERFACES):
    ...
    records[f"local-link-capture-{node}.json"] = {
        ...
        "destination_requests": 4, "destination_responses": 4, "truncated": False,
...
payload = fixture.payload_for(run_id, "client")
digest = hashlib.sha256(payload).hexdigest()
records["local-link-app/client.json"] = {"success": True, "datagrams": 3,
...
records["local-link-app/server.json"] = {"destination": list(fixture.DESTINATION),
    "flows": {"client": {"sha256": digest, "bytes": len(payload), "datagrams": 3,
                         "source_ips": ["10.241.31.1"]}}}
...
with tempfile.TemporaryDirectory(prefix="volparossa-local-link-contract-") as temporary:
    ...
    def evaluate(data, selected_path=path):
    ...
    assert result["success"] and len(result["flows"]) == 1

...
print_plan() {
    ...
    if [ "$scenario" = agent-jobs ]; then
        ...
        if [ "$agent_public_network_sources" = yes ]; then
            printf '%s\n' \
                ...
                '  compile one explicit local source and two exact publisher-authenticated native text sources;' \
                '  reuse one cached source and fetch one missing source through the protected content plane;' \
                '  retain selected publication identities and exact original-source byte provenance;' \
                '  use the isolated tokenizer, two explicit peer workers and shared queues to synthesize one answer;' \
                '  remove original inputs and stop brokers, then resume completed receipts offline;' \
                '  bounded guest-only execution and full cleanup; no web crawl, private source or quality claim.'
            return
        fi
        ...
        printf '%s\n' \
            ...
            '  submit two disjoint signed public rows through protected paths to two real nodes;' \
            '  observe overlapping isolated Python lifetimes, independent locks, exact inputs and replies;' \
            '  stop owned brokers and remove private model/job state; no reassignment or full-B03 claim.'
        return
    fi
    ...
    if [ "$scenario" = mptcp-growth ]; then
        printf '%s\n' \
            ...
            '  select two initial paths and one authorized warm R0/R1/R2 path, with eligible separate R3 control;' \
            '  use one capless ordinary TCP application, the existing release gate and exact 32 MiB download/hash;' \
            '  bound all three disposable relay download legs to the existing fixed 8 Mbps profile;' \
            '  apply fixed 15 percent loss only on one selected disposable Exit xrN download egress;' \
            '  require same-flow kernel tokens/cookies/tuples and fresh ACK/receive deltas on 2 then 3 subflows;' \
            '  require complete private captures and six WireGuard legs, never CLI Reachable as payload evidence;' \
            '  restore only owned qdiscs, disconnect and emit mptcp-growth-smoke.json with unchanged-host cleanup.'
        return
    fi
    ...
    if [ "$scenario" = content-replication ]; then
        ...
        printf '%s\n' \
            'VOLPAROSSA mailbox CLI plan: two Relay3/4/5 provider stores, exact authorized TCP18080 names;' \
            '  independent sender/owner application identities behind the same Client agent; no private keys sent to agents;' \
            '  two exact UDP41000-only broker/provider links, two protected MPTCP relay paths and complete physical captures;' \
            '  normal invite/enroll/send; remove exact disposable sender keys/input; Disconnect and reopen one provider cache;' \
            '  receive on a fresh route without manifest/ID input, verify plaintext and two ACKs, require second receive empty;' \
            '  remove exact temporary secrets/output, stop services and fully clean namespaces; no independent sender-node/full-C07 claim.'
...
if [ "$agent_successor_serving" = yes ] || [ "$agent_active_recovery" = yes ] || [ "$agent_adapter_aggregation" = yes ]; then
...
if [ "$agent_active_recovery" = yes ] || [ "$agent_adapter_aggregation" = yes ]; then
...
if [ "$scenario" = content-mailbox ]; then
    for mailbox_fixture in content-mailbox-smoke.sh content-mailbox-smoke.py content-provider-smoke.sh \
        ...
        if [ ! -f "$source_directory/tests/integration/$mailbox_fixture" ] \
            || [ -L "$source_directory/tests/integration/$mailbox_fixture" ]; then
            printf '%s\n' 'mailbox fixture unavailable' >&2
            exit 69
        fi
    done
    for mailbox_tool in head base64; do
        command -v "$mailbox_tool" >/dev/null 2>&1 || exit 69
    done
fi
...
mptcp_growth_finalize_report() {
    mptcp_status=$1
    mptcp_evidence=$(optional_json_evidence "$WORK/mptcp-growth-evidence.json")
    jq -cn --arg revision "$expected_commit" --arg run_id "$RUN_ID" \
        --arg phase "$PHASE" --arg blocker "$OBSERVED_BLOCKER" \
        --argjson status "$mptcp_status" --argjson evidence "$mptcp_evidence" \
        --argjson topology "$TOPOLOGY_READY" --argjson helpers "$HELPERS_READY" \
        --argjson agents "$AGENTS_READY" --argjson no_direct_exit "$CLIENT_EXIT_ROUTE_ABSENT" \
        --argjson complete "$CLEANUP_COMPLETE" --argjson remaining "$REMAINING_OWNED_OBJECTS" \
        --slurpfile host "$WORK/a15-evidence.json" '
        {schema_version:1,report_kind:"volparossa-mptcp-growth-runtime",
          source_revision:$revision,run_id:$run_id,phase:$phase,
          success:($status == 0 and $evidence.success == true and $complete and
            $remaining == 0 and $host[0].unchanged == true and $topology and
            $helpers and $agents and $no_direct_exit),transfer:$evidence,
          observed_blocker:(if $blocker == "" then null else $blocker end),
          cleanup:{complete:$complete,remaining_owned_objects:$remaining},
          host_state:($host[0] | del(.acceptance_id)),
          scope:"one genuine TLS/MPTCP application flow grows from two to three kernel-data-carrying subflows; no speed or A01-A15 claim"}
    ' >"$WORK/mptcp-growth-smoke.json" || return 1
    for mptcp_artifact in "$WORK"/mptcp-growth-*.json "$WORK"/mptcp-growth-*.txt \
        "$WORK"/mptcp-growth-*.out "$WORK"/mptcp-growth-*.err "$WORK"/mptcp-growth-*.log; do
        if [ ! -f "$mptcp_artifact" ] || [ -L "$mptcp_artifact" ]; then continue; fi
        [ "$(stat -Lc '%s' "$mptcp_artifact")" -le 8388608 ] || return 1
        install -o "$OUTPUT_UID" -g "$OUTPUT_GID" -m 0600 "$mptcp_artifact" \
            "$output_directory/$(basename -- "$mptcp_artifact")" || return 1
    done
    jq -e '.success == true' "$WORK/mptcp-growth-smoke.json" >/dev/null
}
...
if [ "$scenario" = content-mailbox ]; then
    ...
    # shellcheck source=tests/integration/content-provider-smoke.sh
    . "$source_directory/tests/integration/content-provider-smoke.sh"
    # shellcheck source=tests/integration/content-mailbox-smoke.sh
    . "$source_directory/tests/integration/content-mailbox-smoke.sh"
fi
if [ "$scenario" = dns-cache ]; then
...
if [ "$scenario" = content-mailbox ]; then
    for mailbox_script in content-mailbox-smoke.py content-provider-https-smoke.py content-network-smoke.py; do
        install -o root -g root -m 0555 "$source_directory/tests/integration/$mailbox_script" "$WORK/bin/$mailbox_script"
    done
fi
if [ "$scenario" = dns-cache ]; then
...
[ "$scenario" != agent-jobs ] || agent_jobs_prepare
...
elif [ "$scenario" = content-replication ] || [ "$scenario" = content-repair ] || [ "$agent_train_loop" = yes ] || [ "$agent_active_recovery" = yes ] || [ "$agent_adapter_aggregation" = yes ]; then
...
write_config() {
    ...
    if { [ "$agent_successor_serving" = yes ] || [ "$agent_active_recovery" = yes ] || [ "$agent_adapter_aggregation" = yes ]; } && [ "$node" = relay4 ]; then
    ...
    if [ "$scenario" = content-replication ] || [ "$scenario" = content-repair ] || [ "$agent_train_loop" = yes ] || [ "$agent_active_recovery" = yes ] || [ "$agent_adapter_aggregation" = yes ]; then
...
PHASE=helper-launch
CAPABILITIES='CAP_KILL CAP_NET_ADMIN CAP_NET_BIND_SERVICE CAP_SETGID CAP_SETPCAP CAP_SETUID CAP_SYS_ADMIN'
...
launch_agent() {
    ...
    set --
    if { [ "$scenario" = content-provider ] || [ "$scenario" = content-message ] || [ "$scenario" = content-mailbox ] || [ "$scenario" = content-custody ] || [ "$scenario" = agent-artifact ] || [ "$scenario" = agent-jobs ]; } && [ "$node" = client ]; then
...
if [ "$scenario" != mixed-link ] && [ "$scenario" != crash-recovery ] \
    ...
    && [ "$scenario" != content-custody ] && [ "$scenario" != agent-artifact ] && [ "$scenario" != agent-jobs ] && [ "$scenario" != dns-cache ]; then
for node in client bootstrap1 bootstrap2 relay0 relay1 relay2 relay3 relay4 relay5 exit exit2; do

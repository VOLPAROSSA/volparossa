...
local_link_wait_neighbors() {
    ...
    while [ "$(date +%s)" -lt "$local_link_deadline" ]; do
        ...
        "$binary_directory/volparossa" \
            ...
            >"$WORK/local-link-peers-client.txt" || return 1
        # Only C consumes in this scenario. Its two signed adjacent Relay candidates and
        # forwarded remote Exit must exist. A/B/X's own Client-selection caches are unrelated
        # to their service roles and need not contain a usable independent consumption route.
        for local_link_neighbor in "$R0_PEER" "$R2_PEER" "$EXIT_PEER"; do
            awk -v peer="$local_link_neighbor" \
                '$1 == peer && $2 == "roles=0b111" { found=1 } END { exit !found }' \
                "$WORK/local-link-peers-client.txt" || local_link_ready=no
        done
...
local_link_run() {
    ...
    (reciprocity_connect client) &
    local_link_connect_pid=$!
    RECIPROCITY_PIDS="$RECIPROCITY_PIDS $local_link_connect_pid"
    wait "$local_link_connect_pid" || fail LOCAL_LINK_NATIVE_ROUTE_UNAVAILABLE
    ...
    ip netns exec "$CLIENT" setpriv --reuid="$WORKER_UID" --regid="$WORKER_GID" \
        ...
        "$WORK/local-link-app" "$RUN_ID" client >"$WORK/local-link-client.log" 2>&1 &
    local_link_app_pid=$!
    RECIPROCITY_PIDS="$RECIPROCITY_PIDS $local_link_app_pid"
    printf 'go\n' >"$WORK/local-link-app/go"
    reciprocity_wait_file "$WORK/local-link-app/client.active" "$local_link_app_pid" \
    ...
    "$binary_directory/volparossa" --control-socket "$WORK/runtime-client/control/agent.sock" \
        paths >"$WORK/local-link-paths-client.txt"
    ...
    wait "$local_link_app_pid" || fail LOCAL_LINK_APPLICATION_FAILED
...
local_link_finalize_report() {
    ...
    jq -cn --arg revision "$expected_commit" --arg run_id "$RUN_ID" \
        ...
        '$evidence + {schema_version:1,report_kind:"volparossa-local-link-runtime",
          ...
          scope:"one offline-consumer LAN/WAN single-path UDP route; not radio, capacity aggregation or A01-A15"}' \
        >"$WORK/local-link-smoke.json"
    for local_link_artifact in "$WORK"/local-link-*.json "$WORK"/local-link-*.txt \
        "$WORK"/local-link-*.log "$WORK"/reciprocity-connect-client.* \
